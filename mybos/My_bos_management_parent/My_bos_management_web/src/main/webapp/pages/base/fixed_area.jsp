	...
	<head>
		...
		<script type="text/javascript">
			...
			function doAssociations(){
				...
				if(rows.length!=1){
					$.messager.alert("提示信息","只能选一个","warning");					
				}else{
					...
					$.post(
						"${pageContext.request.contextPath}/fixedArea_findNotAssociation.action",
						function(data){
							for(var i=0;i<data.length;i++){
								var custId = data[i].id;
								var info = data[i].username+'('+data[i].telephone+')';
								$("#noassociationSelect").append('<option value="'+custId+'">'+info+'</option>');
								}
						},
						"json"
					);
					...
					var fId=rows[0].id;
					$("#customerFixedAreaId").val(fId);
					$.post(
						"${pageContext.request.contextPath}/fixedArea_findHasAssociationByFixedAreaId.action",
						{fId:fId},
						function(data){
							for(var i=0;i<data.length;i++){
								var custId=data[i].id;
								var show=data[i].username+"("+data[i].telephone+")";
								$("#associationSelect").append("<option value='"+custId+"'>"+show+"</option>");
							}
						},
						"json"
					);
			...
			var toolbar = [ {
				...
				handler : function(){
					...
					if(rows.length==1){
						...
						$("#courierId").combobox({    
						    url:'${pageContext.request.contextPath}/courier_findAll.action',    
						    valueField:'id',    
						    textField:'nameAndNum'   
						});  
			...
			$(function(){
				...
				$('#grid').datagrid( {
					...
					pagination : true,
					toolbar : toolbar,
					url : "${pageContext.request.contextPath}/fixedArea_findPage.action",
					idField : 'id',
					columns : columns,
						...
						<tr>
							...
							<td>
								<table class="easyui-datagrid" style="width:400px;height:250px"   
								        data-options="url:'${pageContext.request.contextPath}/subarea_findAll.action',fitColumns:true,singleSelect:false">   
								    <thead>   
								        <tr>   
								            <th data-options="field:'subareaId',width:100,checkbox:true">编号</th>   
								            <th data-options="field:'keyWords',width:100">关键字</th>   
								            <th data-options="field:'assistKeyWords',width:100,align:'right'">辅助关键字</th>   
								        </tr>   
								    </thead>   
								</table>  
							</td>
					...
					<table class="table-edit" width="80%" align="center">
						...
						<tr>
							...
							<td>
								...
								<input name="courierId" id="courierId" />
							</td>
						</tr>
						<tr>
							<td>选择收派时间</td>
						...
						<tr>
							<td colspan="3">
								<a id="associationCourierBtn" href="javascript:;" class="easyui-linkbutton" data-options="iconCls:'icon-save'">关联快递员</a>
								<script type="text/javascript">
