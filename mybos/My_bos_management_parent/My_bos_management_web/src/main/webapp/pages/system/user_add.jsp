...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function(){
				...
				$.post(
					"${pageContext.request.contextPath}/role_findAll.action",
					function(data){
						for(var i=0;i<data.length;i++){
							
							$("#roleIds").append("<input type='checkbox' name='roleIds' id='"+data[i].id+"' value='"+data[i].id+"'/>"+"<label for='"+data[i].id+"'>"+data[i].name+"</label>");
						}
					}
				);
	...
	<body class="easyui-layout" style="visibility:hidden;">
		...
		<div region="center" style="overflow:auto;padding:5px;" border="false">
			<form action="${pageContext.request.contextPath}/user_save.action" id="userForm" method="post">
